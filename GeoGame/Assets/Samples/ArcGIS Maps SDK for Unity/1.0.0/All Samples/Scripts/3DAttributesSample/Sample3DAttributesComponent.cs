...
[ExecuteAlways]
public class Sample3DAttributesComponent : MonoBehaviour
{
	...
	private void Awake()
	{
		gameMap = GetComponent<GeoGameScript>();
		...
		gameMap.OnSetLayerAttributes += new GeoGameScript.SetLayerAttributesEventHandler(Setup3DAttributes);
	}
	...
	private void Setup3DAttributes(ArcGIS3DObjectSceneLayer buildingLayer)
	{
		...
		if (layerAttribute == AttributeType.YR_BUILT)
		{
			Setup3DAttributesFloatAndIntegerType(buildingLayer);
		}
		else if (layerAttribute == AttributeType.PROP_DESC)
		{
	...
	private int IsBuildingOfInterest(string element)
	{
		...
		else if (element.Equals("Office bldg") || element.Equals("Movie theatr"))
		{
			return 1;
		}
		else
